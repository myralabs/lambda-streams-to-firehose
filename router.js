 ...
 *
 ...
 Look at the record's account_id field.
 If it exists, we route to a firehose router called delivery-stream-accountid.
 If it doesn't, do the default behavior.
...
// attributemap is not needed
function routeByAccountId(attributeMap, defaultDeliveryStreamName, records, callback) {
  var key = ACCOUNT_ID_FIELD;


  if (debug) {
    console.log("\n\n>>> Routing on the basis of Attribute: " + key);
  }
  var routingMap = {};
  ...
  records.map(function(record) {
    try {
      ...
      if (!(key in jsonRecord)) {
	// this object doesn't include account_id, so
	// go to default
	pushArrayValue(defaultDeliveryStreamName, record, routingMap);
	...
	var recordValue = jsonRecord[key];
	var targetDeliveryStream;
	targetDeliveryStream = DELIVERY_STREAM_PREFIX + recordValue;
	if (debug) {
	  console.log(">> Resolved destination Delivery Stream " + targetDeliveryStream + " by value " + recordValue);
	}
	pushArrayValue(targetDeliveryStream, record, routingMap);
      }
    } catch (err) {
...
exports.routeByAccountId = routeByAccountId;
